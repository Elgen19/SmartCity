...
class ActivityDetailsAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ActivityViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_activity_details, parent, false)
...
//            // Set background color to semi-transparent red
//            holder.itemView.setBackgroundColor(Color.argb(77, 255, 0, 0)) // 128 is the alpha value (50% opacity)
//        } else {
//            // Reset background color for other statuses
//            holder.itemView.setBackgroundColor(Color.WHITE) // Or your default color
//        }
        ...
        val statusIcon: ImageView = holder.itemView.findViewById(R.id.stateIcons)
        setStatusIconColor(statusIcon, activity.status)

        toggleStatusIconVisibility(statusIcon, activity.containerStatus)
    }

    private fun setStatusIconColor(imageView: ImageView, status: String?) {
        val context = imageView.context
        val color = when (status) {
            "Active" -> ContextCompat.getColor(context, R.color.active)
            "In Progress" -> ContextCompat.getColor(context, R.color.in_progress)
            "Finished" -> ContextCompat.getColor(context, R.color.done)
            "Upcoming" -> ContextCompat.getColor(context, R.color.upcoming) // Default color
            else -> ContextCompat.getColor(context, R.color.gray)
        }
        imageView.setColorFilter(color, PorterDuff.Mode.SRC_ATOP)
    }

    private fun toggleStatusIconVisibility(imageView: ImageView, containerStatus: String?) {
        imageView.visibility = if (containerStatus == "Scheduled") View.VISIBLE else View.GONE
    }
    ...
    private fun formatTime(time: String): String {
        try {
            ...
            val outputFormat =
                SimpleDateFormat("hh:mm a", Locale.getDefault()) // 12-hour format output with AM/PM
